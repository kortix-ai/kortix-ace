...
def print_aggregate_summary(results: List[RunResult]):
    ...
    print(f"\n  Per-Run Breakdown:")
    print(f"    {'Run':<5} {'Status':<8} {'Duration':<10} {'Steps':<7} {'Tokens':<10} {'Accuracy':<10}")
    print(f"    {'-'*5} {'-'*8} {'-'*10} {'-'*7} {'-'*10} {'-'*10}")
    for r in results:
        status = "OK" if r.success else "FAIL"
        print(f"    {r.run_number:<5} {status:<8} {r.duration_seconds:<10.1f} {r.steps:<7} {r.tokens:<10,} {r.correct_items}/5 ({r.accuracy_pct:.0f}%)")
